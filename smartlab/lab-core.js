...
window.InsightLab = (function () {
  ...
  /* ---------------- GradeNext integration: levels, deep links, embed ----------------
     One engine now serves two tracks inside GradeNext Smart Lab:
       · Higher Secondary — Class 11–12, JEE / NEET (every original InsightVis lab)
       · Middle School    — Grades 6–8, NGSS (labs that declare `grade: 6 | 7 | 8`)
     A lab declares `grade` (a number or an array). A lab that declares none is an
     InsightVis lab and counts as Class 11–12, so the 42 original files stay verbatim.
     The page is opened with ?level=hs | ?level=ms | ?grade=N to show one track, with
     #<labId> to open a lab directly, and with ?embed=1 when GradeNext frames it. */
  function gradesOf(def) {
    const g = def.grade;
    return g == null ? [11, 12] : (Array.isArray(g) ? g : [g]);
  }
  function isHS(def) { return gradesOf(def).some(g => g >= 11); }
  const VIEW = (function () {
    let q = null;
    try { q = new URLSearchParams(location.search); } catch (e) { q = null; }
    const level = q && q.get('level');
    const grade = q ? Number(q.get('grade')) : NaN;
    return {
      level: level === 'hs' || level === 'ms' ? level : null,
      grade: Number.isFinite(grade) && grade > 0 ? grade : null,
      embed: !!(q && q.get('embed') === '1')
    };
  })();
  function inView(def) {
    const gs = gradesOf(def);
    if (VIEW.grade) return gs.indexOf(VIEW.grade) >= 0;
    if (VIEW.level === 'hs') return gs.some(g => g >= 11);
    if (VIEW.level === 'ms') return gs.some(g => g <= 8);
    return true;
  }
  ...
  function viewLabel() {
    ...
    if (VIEW.level === 'hs') return 'Higher Secondary · Class 11–12 · JEE · NEET';
    if (VIEW.level === 'ms') return 'Middle School · Grades 6–8 · NGSS';
    return 'Grades 6–8 and 11–12 · Physics Chemistry Biology';
  }
  ...
  function buildRail() {
    ...
    SUBJECTS.forEach(sub => {
      const list = shown.filter(s => s.subject === sub.id);
  ...
  /* ---------------- mount ---------------- */
  function mount(id) {
    const def = sims.find(s => s.id === id) || visibleSims()[0];
    cancelAnimationFrame(R.raf);
    R.def = def;
    // Keep the address naming the open lab, so it can be shared and reloaded.
    // replaceState does not fire hashchange, so this cannot loop back into mount.
    try {
      if (idFromHash() !== def.id) history.replaceState(null, '', location.pathname + location.search + '#' + encodeURIComponent(def.id));
    } catch (e) { /* file:// or a sandbox that forbids it: the lab still runs */ }
    ...
    tags.innerHTML = '';
    // Higher Secondary labs light up the exams they serve out of the fixed three;
    // a Grades 6–8 lab carries its own tags (NGSS codes) and shows them all lit.
    const exams = def.exams || [];
    (isHS(def) ? ['JEE Main', 'JEE Advanced', 'NEET UG'] : exams).forEach(x =>
      tags.appendChild(el('span', 'tag' + (exams.indexOf(x) >= 0 ? ' on' : ''), x)));
  ...
  function boot() {
    ...
    if (sub) sub.textContent = viewLabel();
